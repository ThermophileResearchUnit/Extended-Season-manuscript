...
envdata_plot <- ggplot(envdata.m, aes(x = Dist.from.bottom, y = value, color = Ice.thickness)) +
  ...
  facet_wrap(. ~ variable, nrow = 3, scales = "free_x",
             labeller = as_labeller(c(DO = "Dissolved Oxygen (mg/L)", 
                                      Conductivity.mS = "Conductivity (mS)",
                                      Temperature = "Temperature (\u00B0C)"))) +
  expand_limits(x = 0) + # Ensures y starts at 0 in all facets
  expand_limits(y = 0) + # Ensures x starts at 0 in all facets
  coord_flip()

envdata_plot

#ggsave("Figure2_env.svg", envdata_plot, width = 180, height = 200, units = "mm",dpi = 300)
#ggsave("Figure2_env.png", envdata_plot, width = 180, height = 200, units = "mm",dpi = 300)
```
