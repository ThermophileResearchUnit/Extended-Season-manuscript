...
esf.clr.rar <- readRDS("physeq.es.clr.rar")
...
esfrel.rar <- readRDS("physeq.es.rel.rar")
...
unifrac.esf.clr.rar <- phyloseq::distance(esf.clr.rar, method = "unifrac")
esf.clr.rar.mds <- ordinate(esf.clr.rar, "MDS", distance = unifrac.esf.clr.rar)

# final plot
pcoa.esf.clr.rar<- plot_ordination(esf.clr.rar, esf.clr.rar.mds, 
                shape="Ice.thickness") +
  geom_point(aes(fill=Dist.from.bottom), size=5,
             alpha = 0.9) +
  #geom_text(label = rownames(esf.clr.rar.mds$vectors),
...
###DO
pcoa.esf.clr.rar.d<- plot_ordination(esf.clr.rar, esf.clr.rar.mds, 
                shape="Ice.thickness") +
  ...
  theme(
    ...
    legend.position = "right",
    legend.box = "vertical") +
  scale_fill_viridis_c(option = "plasma", name = "Dissolved Oxygen (mg/L)") +
  scale_shape_manual(name = "Ice thickness", values = c(21, 22, 23, 24)) +
  guides(color = guide_colorbar(title.position = "top"))  # <-- continuous guide
...
###Conductivity
pcoa.esf.clr.rar.c<- plot_ordination(esf.clr.rar, esf.clr.rar.mds, 
                shape="Ice.thickness") +
...
### 13cm is significantly less rich than 50cm

####Comparing depth bins

shannon_plot2 <- plot_richness(esf.rar, x="Depth.bin",  measures=c("Shannon"))  +
  geom_boxplot(aes(color = Depth.bin)) +
  geom_jitter(size = 2) +
  ylab("Shannon\nIndex") +
...
unifrac.esf.clr.rar.mat <- as.matrix(unifrac.esf.clr.rar)

# PERMANOVA on ice thickness (i.e., season time point)
adonis2(unifrac.esf.clr.rar ~ Ice.thickness, data = envdata)

#dispersion test
disp.site<- betadisper(unifrac.esf.clr.rar, envdata$Ice.thickness)
permutest(disp.site)

# PERMANOVA on depth
adonis2(unifrac.esf.clr.rar ~ Dist.from.bottom, data = envdata)
disp.site<- betadisper(unifrac.esf.clr.rar, envdata$Ice.thickness)
permutest(disp.site)
...
merged_group <- phyloseq::merge_samples(esf.rar,group = "Ice.thickness")
...
pal_barn2 <- RColorBrewer::brewer.pal(4, "Oranges")
pal_barn3 <- RColorBrewer::brewer.pal(4, "YlGn")
pal_barn4 <- RColorBrewer::brewer.pal(3, "Purples")
pal_barnf_c <- c("black", pal_barn2, pal_barn3, pal_barn4)

es_c <- esfrel.rar %>% psmelt() %>%
...
rare_classes <- class_max$Class[class_max$max_abund < 0.005]

# Step 3: relabel
es_c <- es_c %>%
  mutate(Class = ifelse(Class %in% rare_classes, "Class < 0.5 %", Class),
         Class = ifelse(is.na(Class), "Unassigned", Class))


#Reorder class so <0.5% is first, then alphabetical
list <- es_c$Class %>%
  unique(.) %>%
  as.data.frame(.) %>%
  filter(., . != "Unassigned") %>%
...
beta_bara <- ggarrange(pcoa.esf.clr.rar, plot_phylum_a,
                       labels = c("A", "B"),
                       ncol = 2,
                       widths = c(1, 0.5))
...
bio_div_plot2 <- ggarrange(beta_bara, bar2,
                          labels = c("", ""),
                          ncol = 1, nrow = 2,
                          heights = c(0.6, 1))
...
#ggsave("Figure3.png", plot = bio_div_plot2, dpi = 300, width = 240, height = 280, units = "mm")
#ggsave("Figure3.svg", plot = bio_div_plot2, dpi = 300, width = 240, height = 280, units = "mm")

##################
alphas <- ggarrange(shannon_plot, shannon_plot2,
          labels = c("A", "B"),
          ncol = 2, nrow = 1)

alphas

#ggsave("FigureS4.png", plot = alphas, dpi = 300, width = 200, height = 100, units = "mm")
#ggsave("FigureS4.svg", plot = alphas, dpi = 300, width = 200, height = 100, units = "mm")

####Sup fig

extra_pcoa <- ggarrange(pcoa.esf.clr.rar.c, pcoa.esf.clr.rar.d,
          labels = c("A", "B"),
          ncol = 1, nrow = 2)

extra_pcoa
#ggsave("FigureS3.png", plot = extra_pcoa, dpi = 300, width = 200, height = 200, units = "mm")
#ggsave("FigureS3.svg", plot = extra_pcoa, dpi = 300, width = 200, height = 200, units = "mm")
```
```{r}
...
nanos <- readRDS("physeq.es.rel.rar")
...
nano_num # Divide each by num of samples: 13cm: 5; 28 and rest: 6
```
...
#generate Table S1
library(Biostrings)
otu_table <- data.frame(otu_table(esfrel.rar)) %>%
...
tax_table <- data.frame(tax_table(esfrel.rar)) %>%
...
#write.csv(full_df, "Table S2.csv")
```
