...
physeq.es0 <- readRDS("physeq.es")

#remove chimeras identified by uchime
uchime <- read.delim("uchime_out.txt", header = FALSE) %>%
  dplyr::select(V2, V18) %>%
  dplyr::rename("ASV" = V2) %>%
  dplyr::filter(V18 == "Y")

physeq.es <- subset_taxa(physeq.es0, 
                                  !taxa_names(physeq.es0) %in% uchime$ASV)
...
esf <- subset_taxa (physeq.es, (Kingdom !="Eukaryota")) # we remove Eukaryota and NA
...
chlor.reads <- physeq.m %>%
  dplyr::filter(Phylum == "Cyanobacteriota") %>%
  dplyr::summarise(sum(Abundance))

chlor.prop <- chlor.reads / sum(rowSums(otu_table(physeq.es)))
chlor.prop
```
...
colSums.asv.df <- data.frame(colSums(otu_table(esf))) %>%
  tibble::rownames_to_column("ASV")

plot_asvs <- ggplot(colSums.asv.df, aes(x = reorder(ASV, -colSums.otu_table.esf..), y = colSums.otu_table.esf..)) + 
...
rowSums.asv.df <- data.frame(rowSums(otu_table(esf))) %>%
  rownames_to_column("Samples")

plot_reads <- ggplot(rowSums.asv.df, aes(x = reorder(Samples, -rowSums.otu_table.esf..), y =
               rowSums.otu_table.esf..)) +
  geom_bar(stat = "identity") + 
  coord_flip() +
  ylab("Reads") +
  xlab("")
...
summary1 <-
  list("ASVs" = 
         list("min" = ~ min(colSums.otu_table.esf..),
            "median" = ~ median(colSums.otu_table.esf..),
            "max"  = ~ max(colSums.otu_table.esf..))
            )
...
summary2 <-
  list("Reads" = 
         list("min" = ~ min(rowSums.otu_table.esf..),
            "median" = ~ median(rowSums.otu_table.esf..),
            "max"  = ~ max(rowSums.otu_table.esf..))
            )
...
reads_per_sample <- data.frame(Sample = sample_names(esf),
                                TotalReads = sample_sums(esf))
...
rarecurve(otu_mat, step = 1000, col = "blue", label = FALSE)
...
#rarefies to lowest sample depth: 7044 reads

rowSums.asv.df.rar <- data.frame(rowSums(otu_table(esf_rar))) %>%
  rownames_to_column("Samples")
max(rowSums.asv.df.rar$rowSums.otu_table.esf_rar..)

esf_rar_rel <- transform_sample_counts(esf_rar, function(x) x / sum(x) )

unifrac.esf.rel.rar <- phyloseq::distance(esf_rar_rel, method = "unifrac")
esf.rel.mds.rar <- ordinate(esf_rar_rel, "MDS", distance = unifrac.esf.rel.rar)

# final plot
pcoa.esf.rel.rar <- plot_ordination(esf_rar_rel, esf.rel.mds.rar, 
                shape="Ice.thickness") +
  geom_point(aes(fill=Dist.from.bottom), size=5,
             alpha = 0.9) +
  #geom_text(label = rownames(esf.rel.mds.rar$vectors),
...
#non-rarefied data
esf.rel <- transform_sample_counts(esf, function(x) x / sum(x) )
unifrac.esf.rel <- phyloseq::distance(esf.rel, method = "unifrac")
esf.rel.mds <- ordinate(esf.rel, "MDS", distance = unifrac.esf.rel)

# final plot
pcoa.esf.rel<- plot_ordination(esf.rel, esf.rel.mds, 
                shape="Ice.thickness") +
  geom_point(aes(fill=Dist.from.bottom), size=5,
             alpha = 0.9) +
